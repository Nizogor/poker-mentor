//
//  PreFlopStrategyServiceMediumHandTests.swift
//  PokerMentorKitTests
//
//  Created by Nikita Teplyakov on 06.05.2020.
//

import XCTest
@testable import PokerMentorKit

class PreFlopStrategyServiceMediumHandTests: PreFlopStrategyServiceTests {

	lazy var hands = testHandsProvider.allMediumHands()

	// MARK: - First Round

	func testMediumHandStrategy() {
		...
		testOneRaiseAndAtLeastOneCall(cards: cards, position: .early, expectedAction: .fold)
		testOneRaiseAndAtLeastOneCall(cards: cards, position: .middle, expectedAction: .fold)
		testOneRaiseAndAtLeastOneCall(cards: cards, position: .late, expectedAction: .fold)
		testOneRaiseAndAtLeastOneCall(cards: cards, position: .smallBlind, expectedAction: .fold)
